  ...
  return (
    <>
      <h2 className="m-5">
        ...
        <Link
          className="ml-6 px-6 py-3 text-blue-100 no-underline bg-blue-500 rounded hover:bg-blue-600 hover:underline hover:text-blue-200"
          href={{
            pathname: "/postForm",
      ...
      <div className="flex p-3 gap-4 my-3 rounded-xl border-[1px] border-zinc-600 w-3/4">
        <div className="flex flex-col gap-2">
          ...
          <ul className="max-w-6xl space-y-2 text-gray-500 list-disc list-inside dark:text-gray-400">
            {posts.map((post, idx) => (
              <li key={idx} className="text-justify text-base font-light">
                {post.body} / {getCreatedDate(post.createdAt)}
              </li>
            ))}
          </ul>
        </div>
